...
#if (defined(__clang__) || (defined(__GNUC__) && (__GNUC__>=3))) && !defined(LZ4_FORCE_SW_BITCOUNT)
	        return (__builtin_ctzll((U64)val) >> 3);
#else
	        static const int DeBruijnBytePos[64] = { 0, 0, 0, 0, 0, 1, 1, 2, 0, 3, 1, 3, 1, 4, 2, 7, 0, 2, 3, 6, 1, 5, 3
, 5, 1, 3, 4, 4, 2, 5, 6, 7, 7, 0, 1, 2, 3, 3, 4, 6, 2, 6, 5, 5, 3, 4, 5, 6, 7, 1, 2, 4, 6, 4, 4, 5, 7, 2, 6, 5, 7, 6
, 7, 7 };
...
#else
	        static const int DeBruijnBytePos[32] = { 0, 0, 3, 0, 3, 1, 3, 0, 3, 2, 2, 1, 3, 2, 0, 1, 3, 3, 1, 2, 2, 2, 2
, 0, 3, 1, 2, 0, 1, 0, 1, 1 };
...
static void LZ4_putPositionOnHash(const BYTE* p, U32 h, void* tableBase, tableType_t const tableType, const 
BYTE* srcBase)
{
	switch (tableType)
	{
...
const BYTE* LZ4_getPosition(const BYTE* p, void* tableBase, tableType_t tableType, const BYTE* srcBase)
{
	U32 const h = LZ4_hashPosition(p, tableType);
...
static int LZ4_compress_fast_extState(void* state, const char* source, char* dest, int inputSize, int 
...
,                        byU16, noDict, noDictIssue, acceleration);
	        ...
	        return LZ4_compress_generic(ctx, source, dest, inputSize,             0,    notLimited, (sizeof(void*)==8) ? 
...
,                        byU16, noDict, noDictIssue, acceleration);
	        ...
	        return LZ4_compress_generic(ctx, source, dest, inputSize, maxOutputSize, limitedOutput, (sizeof(void*)==8) ? 
byU32 : byPtr, noDict, noDictIssue, acceleration);
	}
}
...
*/
	...
	while (1) {
	    ...
	    if ((length=(token>>ML_BITS)) == RUN_MASK) {
	        ...
	        if ((safeDecode) && unlikely((uptrval)(op)+length<(uptrval)(op))) goto _output_error;   /* overflow 
detection */
	        if ((safeDecode) && unlikely((uptrval)(ip)+length<(uptrval)(ip))) goto _output_error;   /* overflow 
detection */
	    }
	    ...
	    {
	        if (partialDecoding) {
	            if (cpy > oend) goto _output_error;                           /* Error : write attempt beyond end of 
output buffer */
	            if ((endOnInput) && (ip+length > iend)) goto _output_error;   /* Error : read attempt beyond end of 
input buffer */
	        } else {
	            if ((!endOnInput) && (cpy != oend)) goto _output_error;       /* Error : block decoding must stop 
exactly there */
	            if ((endOnInput) && ((ip+length != iend) || (cpy > oend))) goto _output_error;   /* Error : input must 
be consumed */
	        }
	        memcpy(op, ip, length);
	        ip += length;
...
{    
    ...
    Packet_cryptField(&outp,peer->key,startCryptedPortionAt,outp.len - startCryptedPortionAt);
    
    expectReplyTo(Utils_ntoh_u64(*(uint64_t *)&outp.b[0]));
    if (atAddress) {
        ...
        char *tmpAt = InetAddress_toString(atAddress);
        printf("send(HELLO) to destination: %s\n",tmpAt);
        free(tmpAt);
    } else {
        Switch_send(&outp,false); // false == don't encrypt full payload, but add MAC
    }
}
...
void Packet_cryptField(Buffer *outp,const void *key,unsigned int start,unsigned int len)
{
    uint8_t *const data = (uint8_t *)(outp->b);
	uint8_t iv[8];
    ...
    Salsa20 s20;
    Salsa20_init(&s20,key,iv);
    Salsa20_crypt12(&s20,data + start,data + start,len);
}

/**
 * @return True if compressed (result only valid if unencrypted)
 */
bool compressed(Buffer *outp)
{
    return (((outp->b)[ZT_PACKET_IDX_VERB] & ZT_PROTO_VERB_FLAG_COMPRESSED) != 0); 
}

bool Packet_compress(Buffer *outp)
{
	char *const data = (char *)(outp->b);
	char buf[ZT_PROTO_MAX_PACKET_LENGTH * 2];

	if ((!compressed(outp))&&(outp->len > (ZT_PACKET_IDX_PAYLOAD + 64))) { // don't bother compressing tiny packets
		int pl = (int)(outp->len - ZT_PACKET_IDX_PAYLOAD);
		...
		if ((cl > 0)&&(cl < pl)) {
            ...
            outp->len = (unsigned int)cl + ZT_PACKET_IDX_PAYLOAD;
...
{	
	...
	if ((compressed(outp))&&(outp->len >= ZT_PROTO_MIN_PACKET_LENGTH)) {
		if (outp->len > ZT_PACKET_IDX_PAYLOAD) {
			unsigned int compLen = outp->len - ZT_PACKET_IDX_PAYLOAD;
			int ucl = LZ4_decompress_safe((const char *)data + ZT_PACKET_IDX_PAYLOAD,buf,compLen,sizeof(buf));
			if ((ucl > 0)&&(ucl <= (int)(1024*50 - ZT_PACKET_IDX_PAYLOAD))) {
                outp->len = (unsigned int)ucl + ZT_PACKET_IDX_PAYLOAD;
