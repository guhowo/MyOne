...
static inline void C25519_select(unsigned int p[64],unsigned int q[64],const unsigned int r[64],const unsigned int s[64],unsigned int b)
{
  unsigned int j;
  unsigned int t;
...
static inline crypto_uint32 equal_crypto_uint32(crypto_uint32 a,crypto_uint32 b) /* 16-bit inputs */
{
...
static inline void reduce_add_sub_fe25519(fe25519 *r)
{
  crypto_uint32 t;
  int i,rep;
...
{
  int i;
  crypto_uint32 m = equal_crypto_uint32(r->v[31],127);
  for(i=30;i>0;i--)
    m &= equal_crypto_uint32(r->v[i],255);
  m &= ge(r->v[0],237);
...
static inline void reduce_add_sub_sc25519(sc25519 *r)
{
  crypto_uint32 pb = 0;
  crypto_uint32 b;
  crypto_uint32 mask;
...
static inline unsigned char equal_char(signed char b,signed char c)
{
  unsigned char ub = b;
  unsigned char uc = c;
...
static inline void choose_t(ge25519_aff *t, unsigned long long pos, signed char b)
{
  ...
  *t = ge25519_base_multiples_affine[5*pos+0];
  cmov_aff(t, &ge25519_base_multiples_affine[5*pos+1],equal_char(b,1) | equal_char(b,-1));
  cmov_aff(t, &ge25519_base_multiples_affine[5*pos+2],equal_char(b,2) | equal_char(b,-2));
  cmov_aff(t, &ge25519_base_multiples_affine[5*pos+3],equal_char(b,3) | equal_char(b,-3));
  cmov_aff(t, &ge25519_base_multiples_affine[5*pos+4],equal_char(b,-4));
  fe25519_neg(&v, &t->x);
...
static inline void get_hram(unsigned char *hram, const unsigned char *sm, const unsigned char *pk, unsigned char *playground, unsigned long long smlen)
{
  ...
  SHA512_hash(hram,playground,(unsigned int)smlen);
}
...
void C25519_sign(const Private myPrivate,const Public myPublic,const void *msg,unsigned int len,void *signature)
{
  ...
  SHA512_hash(digest,msg,len);

  SHA512_hash(extsk,myPrivate + 32,32);
  extsk[0] &= 248;
  extsk[31] &= 127;
  extsk[31] |= 64;
...
int C25519_verify(const Public their,const void *msg,unsigned int len,const void *signature)
{
  ...
  SHA512_hash(digest,msg,len);
  if (!Utils_secureEq(sig + 64,digest,32))
    return false;
  ...
  return Utils_secureEq(sig,t2,32);
}
...
void C25519_calcPubDH(Pair *kp)
{
  ...
  crypto_scalarmult_base(kp->pub,kp->priv);
}
...
void C25519_calcPubED(Pair *kp)
{
  ...
  SHA512_hash(extsk,kp->priv + 32,32);
  extsk[0] &= 248;
  extsk[31] &= 127;
  extsk[31] |= 64;
  sc25519_from32bytes(&scsk,extsk);
  ge25519_scalarmult_base(&gepk,&scsk);
  ge25519_pack(kp->pub + 32,&gepk);
