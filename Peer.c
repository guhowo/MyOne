...
{
	if ((now - peer->lastEchoRequestReceived) >= ZT_PEER_GENERAL_RATE_LIMIT) {
		peer->lastEchoRequestReceived = now;
		return true;
	}
	return false;
}
...
bool Peer_hasActivePathTo(Peer *peer, uint64_t now,const InetAddress *addr)
{
    ...
    if((addr->address.ss_family == AF_INET)&&(peer->v4Path.p)){
        if(InetAddress_ipsEqual(&peer->v4Path.p->addr, addr)){
            if(Path_alive(peer->v4Path.p, now)){
                ret = true;
            }
        }
    }else if((addr->address.ss_family == AF_INET6)&&(peer->v6Path.p)){
        if(InetAddress_ipsEqual(&peer->v6Path.p->addr, addr)){
            if(Path_alive(peer->v6Path.p, now)){
                ret = true;
            }
        }
    }
