...
struct _s20sseconsts{
    __m128i maskLo32,maskHi32;
};
void _s20sseconsts_init(struct _s20sseconsts *s){
    s->maskLo32 = _mm_shuffle_epi32(_mm_cvtsi32_si128(-1), _MM_SHUFFLE(1, 0, 1, 0));
    s->maskHi32 = _mm_slli_epi64(s->maskLo32, 32);
    return;
}

static struct _s20sseconsts _S20SSECONSTANTS;

#endif

void Salsa20_init(Salsa20 *s20, const void *key,const void *iv)
{
#ifdef ZT_SALSA20_SSE
    _s20sseconsts_init(&_S20SSECONSTANTS);
    const uint32_t *const k = (const uint32_t *)key;
    s20->_state.i[0] = 0x61707865;
    s20->_state.i[1] = 0x3320646e;
    s20->_state.i[2] = 0x79622d32;
    s20->_state.i[3] = 0x6b206574;
    s20->_state.i[4] = k[3];
    s20->_state.i[5] = 0;
    s20->_state.i[6] = k[7];
    s20->_state.i[7] = k[2];
    ...
    s20->_state.i[9] = k[6];
    s20->_state.i[10] = k[1];
    s20->_state.i[11] = ((const uint32_t *)iv)[1];
    s20->_state.i[12] = k[5];
    s20->_state.i[13] = k[0];
    s20->_state.i[14] = ((const uint32_t *)iv)[0];
    s20->_state.i[15] = k[4];
#else
    const char *const constants = "expand 32-byte k";
    const uint8_t *const k = (const uint8_t *)key;
    s20->_state.i[0] = U8TO32_LITTLE(constants + 0);
    s20->_state.i[1] = U8TO32_LITTLE(k + 0);
    s20->_state.i[2] = U8TO32_LITTLE(k + 4);
    s20->_state.i[3] = U8TO32_LITTLE(k + 8);
    s20->_state.i[4] = U8TO32_LITTLE(k + 12);
    s20->_state.i[5] = U8TO32_LITTLE(constants + 4);
    s20->_state.i[6] = U8TO32_LITTLE(((const uint8_t *)iv) + 0);
    s20->_state.i[7] = U8TO32_LITTLE(((const uint8_t *)iv) + 4);
    s20->_state.i[8] = 0;
    s20->_state.i[9] = 0;
    s20->_state.i[10] = U8TO32_LITTLE(constants + 8);
    s20->_state.i[11] = U8TO32_LITTLE(k + 16);
    s20->_state.i[12] = U8TO32_LITTLE(k + 20);
    s20->_state.i[13] = U8TO32_LITTLE(k + 24);
    s20->_state.i[14] = U8TO32_LITTLE(k + 28);
    s20->_state.i[15] = U8TO32_LITTLE(constants + 12);
#endif

    return;
}

void Salsa20_crypt12(Salsa20 *s20, const void *in,void *out,unsigned int bytes)
{
    uint8_t tmp[64];
    ...
    for (;;) {
...
#ifdef ZT_SALSA20_SSE
        __m128i X0 = _mm_loadu_si128((const __m128i *)&(s20->_state.v[0]));
        __m128i X1 = _mm_loadu_si128((const __m128i *)&(s20->_state.v[1]));
        __m128i X2 = _mm_loadu_si128((const __m128i *)&(s20->_state.v[2]));
        __m128i X3 = _mm_loadu_si128((const __m128i *)&(s20->_state.v[3]));
        __m128i T;
        __m128i X0s = X0;
        __m128i X1s = X1;
        __m128i X2s = X2;
        ...
        _mm_storeu_ps((float *)(c),_mm_castsi128_ps(_mm_xor_si128(_mm_unpackhi_epi64(k02,k20),_mm_castps_si128(_mm_loadu_ps((const float *)(m))))));
        _mm_storeu_ps((float *)(c) + 4,_mm_castsi128_ps(_mm_xor_si128(_mm_unpackhi_epi64(k13,k31),_mm_castps_si128(_mm_loadu_ps((const float *)(m) + 4)))));
        _mm_storeu_ps((float *)(c) + 8,_mm_castsi128_ps(_mm_xor_si128(_mm_unpacklo_epi64(k20,k02),_mm_castps_si128(_mm_loadu_ps((const float *)(m) + 8)))));
        _mm_storeu_ps((float *)(c) + 12,_mm_castsi128_ps(_mm_xor_si128(_mm_unpacklo_epi64(k31,k13),_mm_castps_si128(_mm_loadu_ps((const float *)(m) + 12)))));

        if (!(++s20->_state.i[8])) {
            ++s20->_state.i[5]; // state reordered for SSE
...
void Salsa20_crypt20(Salsa20 *s20,const void *in,void *out,unsigned int bytes)
{
    uint8_t tmp[64];
    const uint8_t *m = (const uint8_t *)in;
    uint8_t *c = (uint8_t *)out;
